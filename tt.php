...
$sql =("SELECT faculty_code,co_outcome,course_code,co_range,programme_code,depart_code from co  where faculty_code = '".$userData['faculty_code']."'");
...
$pdf->Setx($pdf->lMargin);
$pdf->cell(45,120,$row["course_code"],0,0,'R');
...
$pdf->Setx($pdf->lMargin);
$pdf->cell(35,100,$row1["department"],0,0,'R');
...
while($row = mysql_fetch_array($retval))
{
  ...
  $pdf->SetFont('Arial','B',12);
$pdf->Setx($pdf->lMargin);
$pdf->Cell(150,200,"           *".$row["co_range"]."-",0,0,'L');
$pdf->Setx($pdf->lMargin);
$pdf->cell(100,200,"                                            ".$row["co_outcome"],0,0,'L');
$pdf->WriteHTML("<BR><BR>");
}
